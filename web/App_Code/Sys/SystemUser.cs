...
public class SystemUser
{
    ...
    public static SystemUser Login(string username, string password)
    {
        ...
        {
            string sqlStr = "select User_ID YH_ID, LoginName YH_DLM,User_XM YH_XM,Password from tb_b_Users where  User_DelFlag=0 and User_Enable=0 and LoginName=@LoginName and Password=@Password";
    ...
    public void ModifyPassword(string oldPassword, string newPassword)
    {
        ...
        using (DBConnection dbc = new DBConnection())
        {
            try
            {
                MySqlCommand cmd = new MySqlCommand("update tb_b_Users set Password = @newpwd, updatetime=getdate() where User_ID=@yh_id and Password=@oldpwd");
    ...
    {
        string sqlStr = "select User_ID YH_ID, LoginName YH_DLM,User_XM YH_XM,QY_ID,User_DM from tb_b_Users  where User_DelFlag=0 and User_ID = @yh_id";
        MySqlCommand cmd = new MySqlCommand(sqlStr);
        cmd.Parameters.AddWithValue("@yh_id", userid);
        using (DBConnection dbc = new DBConnection())
        {
    ...
    public bool CheckPrivilege(params Guid[] pids)
    {
        ...
        if (pids != null)
        {
            ...
            for (int i = 0; i <= pids.Length - 1; i++)
            {
                ...
                sqlCmd.Append(i);
                cmd.Parameters.Add("@PrivilegeCode" + i, MySqlDbType.VarChar).Value = pids[i];
