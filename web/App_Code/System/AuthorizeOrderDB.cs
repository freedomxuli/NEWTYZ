...
namespace AuthorizeOrderDB
{
    ...
    public class AuthorizeOrderDB
    {
            ...
            using (DBConnection dbc = new DBConnection(ConfigurationManager.ConnectionStrings["LockConnStr"].ConnectionString))
            {
                try
                {
                    ...
                    string sqlStr = @"select a.*,b.HotelName,b.Mobile,b.CompleteAddress from Lock_AuthorizeOrder a
                                 ...
                                 where 1=1 " + mStr + " order by a.ID desc";
                    if (type == 1 || type == 4)
                    {
                        sqlStr = @"select a.*,b.HotelName,b.Mobile,b.CompleteAddress from Lock_AuthorizeOrder a
                                 left join Lock_Hotel b on a.HotelId=b.ID
                                 where AuthorStatus=1 and AuthorizeBookStatus=1 " + mStr + "  order by a.ID desc";
                    }
                    if (type == 2)
                    {
                        sqlStr = @"select a.*,b.HotelName,b.Mobile,b.CompleteAddress from Lock_AuthorizeOrder a
                                 left join Lock_Hotel b on a.HotelId=b.ID
                                 where HandlerKind=1 and OrderUserState=0 and AuthorLiveStatus=1 " + mStr + "  order by a.ID desc";
                    }
                    if (type == 3)
                    {
                        sqlStr = @"select a.*,b.HotelName,b.Mobile,b.CompleteAddress from Lock_AuthorizeOrder a
                                 left join Lock_Hotel b on a.HotelId=b.ID
                                 where AuthorStatus=3  and AuthorLiveStatus=6 " + mStr + "   order by a.ID desc";
                    }
        ...
        public static int GetAuthorizeOrderNum(int type)
        {
            using (DBConnection dbc = new DBConnection(ConfigurationManager.ConnectionStrings["LockConnStr"].ConnectionString))
            {
                try
                {
                    ...
                    DataTable dtPage = dbc.ExecuteDataTable(sqlStr);
                    return dtPage.Rows.Count;
                }
                catch (Exception ex)
                {
                    throw ex;
        ...
        public object GetHotelCombobox()
        {
            using (DBConnection dbc = new DBConnection(ConfigurationManager.ConnectionStrings["LockConnStr"].ConnectionString))
            {
                try
                {
                    string sqlStr = "select ID VALUE,HotelName TEXT from Lock_Hotel  order by HotelName";
        ...
        public object GetRoomCombobox(int hotelId)
        {
            using (DBConnection dbc = new DBConnection(ConfigurationManager.ConnectionStrings["LockConnStr"].ConnectionString))
            {
                try
                {
                    string sqlStr = "select ID VALUE,RoomNo TEXT from Lock_Room where  HotelId=" + hotelId + " order by  RoomNo";
                    DataTable dt = dbc.ExecuteDataTable(sqlStr);
                    return dt;
                }
                catch (Exception ex)
